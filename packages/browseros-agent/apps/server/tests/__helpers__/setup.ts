...
async function waitForExtensionConnection(
  port: number,
  // Extension startup can be slow on a cold ThriveOS profile.
...
/**
 * Ensures the full ThriveOS test environment is ready:
 ...
 */
export async function ensureThriveOS(
  ...
  console.log('\n=== Setting up ThriveOS test environment ===')
...
/**
 * Cleans up the full ThriveOS test environment.
 */
export async function cleanupThriveOS(): Promise<void> {
  console.log('\n=== Cleaning up ThriveOS test environment ===')
  await killBrowser()
  await killServer()
  runtimePlan = null
