...
import { ThriveOSController } from './ThriveOSController'

const STATS_LOG_INTERVAL_MS = 30000

interface ControllerState {
  controller: ThriveOSController | null
  initPromise: Promise<ThriveOSController> | null
  statsTimer: ReturnType<typeof setInterval> | null
}

type ThriveOSGlobals = typeof globalThis & {
  __browserosControllerState?: ControllerState
  __browserosController?: ThriveOSController | null
}

const globals = globalThis as ThriveOSGlobals
...
function setDebugController(controller: ThriveOSController | null): void {
...
async function getOrCreateController(): Promise<ThriveOSController> {
  ...
  if (!controllerState.initPromise) {
    controllerState.initPromise = (async () => {
      try {
        ...
        const controller = new ThriveOSController(getWebSocketPort)
