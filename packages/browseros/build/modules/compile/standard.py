#!/usr/bin/env python3
"""Standard single-architecture build module for ThriveOS"""
...
class CompileModule(CommandModule):
    produces = ["built_app"]
    requires = []
    description = "Build ThriveOS using autoninja"
    ...
    def validate(self, ctx: Context) -> None:
        ...
        if not ctx.browseros_chromium_version:
            raise ValidationError("ThriveOS chromium version not set")
    ...
    def execute(self, ctx: Context) -> None:
        log_info("\n🔨 Building ThriveOS (this will take a while)...")
