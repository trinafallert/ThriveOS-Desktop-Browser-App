...
Command-line interface for controlling ThriveOS via MCP. Talks to the ThriveOS MCP server over JSON-RPC 2.0 / StreamableHTTP.
...
**ThriveOS → Settings → ThriveOS MCP → Server URL**
...
Integration tests require a running ThriveOS server with the dev build (for structured content support).
...
./browseros-cli init
# Enter the Server URL shown in ThriveOS settings
...
Tests skip gracefully if no server is reachable — they won't fail in environments without ThriveOS.
...
The CLI communicates with ThriveOS via two HTTP POST requests per command:
