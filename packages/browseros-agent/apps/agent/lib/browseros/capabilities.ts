import { ThriveOSAdapter } from './adapter'

const SERVER_VERSION_PREF = 'browseros.server.version'

type FeatureConfig = {
  minThriveOSVersion?: string
  maxThriveOSVersion?: string
  minServerVersion?: string
  maxServerVersion?: string
}

/**
 * Features gated by ThriveOS version.
 ...
 * - minThriveOSVersion: feature enabled when ThriveOS >= this version
 * - maxThriveOSVersion: feature enabled when ThriveOS < this version (for deprecation)
...
const FEATURE_CONFIG: { [K in Feature]: FeatureConfig } = {
  [Feature.OPENAI_COMPATIBLE_SUPPORT]: { minThriveOSVersion: '0.33.0.1' },
  [Feature.MANAGED_MCP_SUPPORT]: { minThriveOSVersion: '0.34.0.0' },
  [Feature.PERSONALIZATION_SUPPORT]: { minThriveOSVersion: '0.36.1.0' },
  [Feature.UNIFIED_PORT_SUPPORT]: { minThriveOSVersion: '0.36.1.0' },
  [Feature.CUSTOMIZATION_SUPPORT]: { minThriveOSVersion: '0.36.1.0' },
  [Feature.WORKSPACE_FOLDER_SUPPORT]: { minThriveOSVersion: '0.36.4.0' },
  [Feature.PROXY_SUPPORT]: { minThriveOSVersion: '0.39.0.1' },
  ...
  [Feature.NEWTAB_CHAT_SUPPORT]: { minThriveOSVersion: '0.40.0.0' },
  [Feature.VERTICAL_TABS_SUPPORT]: { minThriveOSVersion: '0.42.0.0' },
  [Feature.MEMORY_SUPPORT]: { minServerVersion: '0.0.73' },
  [Feature.SKILLS_SUPPORT]: { minThriveOSVersion: '0.43.0.0' },
...
async function doInitialize(): Promise<CapabilitiesState> {
  const adapter = ThriveOSAdapter.getInstance()
  ...
  try {
    ...
    // ThriveOS version unknown - features requiring it will be disabled
  }
...
function checkFeatureSupport(
  ...
  const hasThriveOSConstraints =
    config.minThriveOSVersion || config.maxThriveOSVersion
  if (
    hasThriveOSConstraints &&
    !checkVersionConstraints(
      state.browserOSVersion,
      config.minThriveOSVersion,
      config.maxThriveOSVersion,
    )
  ) {
    return false
  }
...
export const Capabilities = {
  ...
  async getThriveOSVersion(): Promise<string | null> {
    const state = await ensureInitialized()
