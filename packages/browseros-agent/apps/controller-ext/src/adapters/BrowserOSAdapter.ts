...
export class ThriveOSAdapter {
   ...
   */
  static getInstance(): ThriveOSAdapter {
    if (!ThriveOSAdapter.instance) {
      ThriveOSAdapter.instance = new ThriveOSAdapter()
    }
  ...
  async getInteractiveSnapshot(
    ...
    try {
      logger.debug(
        `[ThriveOSAdapter] Getting interactive snapshot for tab ${tabId} with options: ${JSON.stringify(options)}`,
      )
          ...
          chrome.browserOS.getInteractiveSnapshot(
            ...
            (snapshot: InteractiveSnapshot) => {
              if (chrome.runtime.lastError) {
                ...
                logger.debug(
                  `[ThriveOSAdapter] Retrieved snapshot with ${snapshot.elements.length} elements`,
                )
                resolve(snapshot)
              }
            },
      ...
      logger.error(
        `[ThriveOSAdapter] Failed to get interactive snapshot: ${errorMessage}`,
      )
  ...
  async click(tabId: number, nodeId: number): Promise<void> {
    try {
      logger.debug(`[ThriveOSAdapter] Clicking node ${nodeId} in tab ${tabId}`)
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error)
      logger.error(`[ThriveOSAdapter] Failed to click node: ${errorMessage}`)
  ...
  async inputText(tabId: number, nodeId: number, text: string): Promise<void> {
    try {
      logger.debug(
        `[ThriveOSAdapter] Inputting text into node ${nodeId} in tab ${tabId}`,
      )
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error)
      logger.error(`[ThriveOSAdapter] Failed to input text: ${errorMessage}`)
      throw new Error(
  ...
  async clear(tabId: number, nodeId: number): Promise<void> {
    try {
      logger.debug(`[ThriveOSAdapter] Clearing node ${nodeId} in tab ${tabId}`)
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error)
      logger.error(`[ThriveOSAdapter] Failed to clear node: ${errorMessage}`)
  ...
  async scrollToNode(tabId: number, nodeId: number): Promise<boolean> {
    try {
      logger.debug(
        `[ThriveOSAdapter] Scrolling to node ${nodeId} in tab ${tabId}`,
      )
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error)
      logger.error(
        `[ThriveOSAdapter] Failed to scroll to node: ${errorMessage}`,
      )
  ...
  async sendKeys(tabId: number, keys: chrome.browserOS.Key): Promise<void> {
    try {
      logger.debug(`[ThriveOSAdapter] Sending keys "${keys}" to tab ${tabId}`)
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error)
      logger.error(`[ThriveOSAdapter] Failed to send keys: ${errorMessage}`)
  ...
  async getPageLoadStatus(tabId: number): Promise<PageLoadStatus> {
    try {
      logger.debug(
        `[ThriveOSAdapter] Getting page load status for tab ${tabId}`,
      )
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error)
      logger.error(
        `[ThriveOSAdapter] Failed to get page load status: ${errorMessage}`,
      )
  ...
  async getAccessibilityTree(
    ...
    try {
      logger.debug(
        `[ThriveOSAdapter] Getting accessibility tree for tab ${tabId}`,
      )
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error)
      logger.error(
        `[ThriveOSAdapter] Failed to get accessibility tree: ${errorMessage}`,
      )
  ...
  async captureScreenshot(
    ...
    try {
      ...
      logger.debug(
        `[ThriveOSAdapter] Capturing screenshot for tab ${tabId}${sizeDesc}${highlightDesc}${dimensionsDesc}`,
      )
        ...
        if (width !== undefined && height !== undefined) {
          chrome.browserOS.captureScreenshot(
            ...
            (dataUrl: string) => {
              if (chrome.runtime.lastError) {
                ...
                logger.debug(
                  `[ThriveOSAdapter] Screenshot captured for tab ${tabId} (${width}x${height})${highlightDesc}`,
                )
                resolve(dataUrl)
              }
            },
            ...
            chrome.browserOS.captureScreenshot(
              ...
              (dataUrl: string) => {
                if (chrome.runtime.lastError) {
                  ...
                  logger.debug(
                    `[ThriveOSAdapter] Screenshot captured for tab ${tabId}${sizeDesc}${highlightDesc}`,
                  )
                  resolve(dataUrl)
                }
              },
            ...
            chrome.browserOS.captureScreenshot(
              ...
              (dataUrl: string) => {
                if (chrome.runtime.lastError) {
                  ...
                  logger.debug(
                    `[ThriveOSAdapter] Screenshot captured for tab ${tabId} (${size}: ${pixelSize}px)`,
                  )
                  resolve(dataUrl)
                }
              },
          ...
          chrome.browserOS.captureScreenshot(tabId, (dataUrl: string) => {
            if (chrome.runtime.lastError) {
              ...
              logger.debug(
                `[ThriveOSAdapter] Screenshot captured for tab ${tabId}`,
              )
              resolve(dataUrl)
            }
          })
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error)
      logger.error(
        `[ThriveOSAdapter] Failed to capture screenshot: ${errorMessage}`,
      )
  ...
  async getSnapshot(tabId: number, _type: SnapshotType): Promise<Snapshot> {
    try {
      logger.debug(`[ThriveOSAdapter] Getting snapshot for tab ${tabId}`)
      ...
      const promise = new Promise<Snapshot>((resolve, reject) => {
        chrome.browserOS.getSnapshot(tabId, (snapshot: Snapshot) => {
          if (chrome.runtime.lastError) {
            ...
            logger.debug(
              `[ThriveOSAdapter] Retrieved snapshot: ${JSON.stringify(snapshot)}`,
            )
            resolve(snapshot)
          }
        })
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error)
      logger.error(`[ThriveOSAdapter] Failed to get snapshot: ${errorMessage}`)
  ...
  /**
   * Generic method to invoke any ThriveOS API
   * Useful for future APIs or experimental features
   */
  async invokeAPI(method: string, ...args: unknown[]): Promise<unknown> {
    try {
      logger.debug(`[ThriveOSAdapter] Invoking ThriveOS API: ${method}`)

      if (!(method in chrome.browserOS)) {
        throw new Error(`Unknown ThriveOS API method: ${method}`)
      }
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error)
      logger.error(
        `[ThriveOSAdapter] Failed to invoke API ${method}: ${errorMessage}`,
      )
      throw new Error(
        `Failed to invoke ThriveOS API ${method}: ${errorMessage}`,
      )
    }
  }
  ...
  /**
   * Get list of available ThriveOS APIs
   */
  ...
  /**
   * Get ThriveOS version information
   */
  async getVersion(): Promise<string | null> {
    try {
      logger.debug('[ThriveOSAdapter] Getting ThriveOS version')
        ...
        if (
          ...
          chrome.browserOS.getVersionNumber((version: string) => {
            if (chrome.runtime.lastError) {
              ...
              logger.debug(`[ThriveOSAdapter] ThriveOS version: ${version}`)
              resolve(version)
            }
          })
        } else {
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error)
      logger.error(`[ThriveOSAdapter] Failed to get version: ${errorMessage}`)
  ...
  async logMetric(
    ...
    try {
      logger.debug(
        `[ThriveOSAdapter] Logging metric: ${eventName} with properties: ${JSON.stringify(properties)}`,
      )
      ...
      return new Promise<void>((resolve, reject) => {
        ...
        if (
          ...
          if (properties) {
            chrome.browserOS.logMetric(eventName, properties, () => {
              if (chrome.runtime.lastError) {
                ...
                logger.debug(`[ThriveOSAdapter] Metric logged: ${eventName}`)
                resolve()
              }
            })
          } else {
          ...
          logger.warn(
            `[ThriveOSAdapter] logMetric API not available, skipping metric: ${eventName}`,
          )
          resolve()
        }
      })
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error)
      logger.error(`[ThriveOSAdapter] Failed to log metric: ${errorMessage}`)
      return
    }
  }
  ...
  async executeJavaScript(tabId: number, code: string): Promise<unknown> {
    try {
      logger.debug(`[ThriveOSAdapter] Executing JavaScript in tab ${tabId}`)
        ...
        if (
          ...
          chrome.browserOS.executeJavaScript(tabId, code, (result: unknown) => {
            if (chrome.runtime.lastError) {
              ...
              logger.debug(
                `[ThriveOSAdapter] JavaScript executed successfully in tab ${tabId}`,
              )
              resolve(result)
            }
          })
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error)
      logger.error(
        `[ThriveOSAdapter] Failed to execute JavaScript: ${errorMessage}`,
      )
  ...
  async clickCoordinates(tabId: number, x: number, y: number): Promise<void> {
    try {
      logger.debug(
        `[ThriveOSAdapter] Clicking at coordinates (${x}, ${y}) in tab ${tabId}`,
      )
        ...
        if (
          ...
          chrome.browserOS.clickCoordinates(tabId, x, y, () => {
            if (chrome.runtime.lastError) {
              ...
              logger.debug(
                `[ThriveOSAdapter] Successfully clicked at (${x}, ${y}) in tab ${tabId}`,
              )
              resolve()
            }
          })
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error)
      logger.error(
        `[ThriveOSAdapter] Failed to click at coordinates: ${errorMessage}`,
      )
      throw new Error(
  ...
  async typeAtCoordinates(
    ...
    try {
      logger.debug(
        `[ThriveOSAdapter] Typing at coordinates (${x}, ${y}) in tab ${tabId}`,
      )
        ...
        if (
          ...
          chrome.browserOS.typeAtCoordinates(tabId, x, y, text, () => {
            if (chrome.runtime.lastError) {
              ...
              logger.debug(
                `[ThriveOSAdapter] Successfully typed "${text}" at (${x}, ${y}) in tab ${tabId}`,
              )
              resolve()
            }
          })
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error)
      logger.error(
        `[ThriveOSAdapter] Failed to type at coordinates: ${errorMessage}`,
      )
      throw new Error(
  ...
  async getPref(name: string): Promise<PrefObject> {
    try {
      console.log(`[ThriveOSAdapter] Getting preference: ${name}`)
      ...
      return new Promise<PrefObject>((resolve, reject) => {
        chrome.browserOS.getPref(name, (pref: PrefObject) => {
          if (chrome.runtime.lastError) {
            ...
            console.log(
              `[ThriveOSAdapter] Retrieved preference ${name}: ${JSON.stringify(pref)}`,
            )
            resolve(pref)
          }
        })
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error)
      console.error(
        `[ThriveOSAdapter] Failed to get preference: ${errorMessage}`,
      )
  ...
  async setPref(
    ...
    try {
      console.log(
        `[ThriveOSAdapter] Setting preference ${name} to ${JSON.stringify(value)}`,
      )
        ...
        if (pageId !== undefined) {
          chrome.browserOS.setPref(name, value, pageId, (success: boolean) => {
            if (chrome.runtime.lastError) {
              ...
              console.log(
                `[ThriveOSAdapter] Successfully set preference ${name}`,
              )
              resolve(success)
            }
          })
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error)
      console.error(
        `[ThriveOSAdapter] Failed to set preference: ${errorMessage}`,
      )
  ...
  async getAllPrefs(): Promise<PrefObject[]> {
    try {
      console.log('[ThriveOSAdapter] Getting all preferences')
      ...
      return new Promise<PrefObject[]>((resolve, reject) => {
        chrome.browserOS.getAllPrefs((prefs: PrefObject[]) => {
          if (chrome.runtime.lastError) {
            ...
            console.log(
              `[ThriveOSAdapter] Retrieved ${prefs.length} preferences`,
            )
            resolve(prefs)
          }
        })
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error)
      console.error(
        `[ThriveOSAdapter] Failed to get all preferences: ${errorMessage}`,
      )
...
export const getThriveOSAdapter = () => ThriveOSAdapter.getInstance()
