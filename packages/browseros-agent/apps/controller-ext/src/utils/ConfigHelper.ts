...
import { getThriveOSAdapter } from '@/adapters/ThriveOSAdapter'
import { WEBSOCKET_CONFIG } from '@/config/constants'
import { logger } from '@/utils/logger'

/**
 * Get the WebSocket port from ThriveOS preferences
...
export async function getWebSocketPort(): Promise<number> {
  try {
    ...
    if (pref && typeof pref.value === 'number') {
      logger.info(`Using port from ThriveOS preferences: ${pref.value}`)
      return pref.value
    }
    ...
    logger.error(
      `Failed to get port from ThriveOS preferences: ${error}, using default: ${WEBSOCKET_CONFIG.defaultExtensionPort}`,
    )
    return WEBSOCKET_CONFIG.defaultExtensionPort
  }
}
