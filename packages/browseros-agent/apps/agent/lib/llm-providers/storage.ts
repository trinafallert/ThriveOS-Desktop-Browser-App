...
import { getThriveOSAdapter } from '@/lib/browseros/adapter'
...
const DEFAULT_PROVIDER_NAME = 'ThriveOS'
...
/** Backup providers to ThriveOS prefs (write-only, best-effort) */
async function backupToThriveOS(backup: LlmProvidersBackup): Promise<void> {
  try {
    const adapter = getThriveOSAdapter()
    await adapter.setPref(BROWSEROS_PREFS.PROVIDERS, JSON.stringify(backup))
  } catch {
    // ThriveOS API not available - ignore
  }
}

/**
 * Setup one-way sync of LLM providers to ThriveOS prefs
 * @public
 */
export function setupLlmProvidersBackupToThriveOS(): () => void {
  const unsubscribe = providersStorage.watch(async (providers) => {
    if (providers) {
      const defaultProviderId = await defaultProviderIdStorage.getValue()
      await backupToThriveOS({ defaultProviderId, providers })
    }
  })
  return unsubscribe
}
...
/** Creates the default ThriveOS provider configuration */
export function createDefaultThriveOSProvider(): LlmProviderConfig {
  const timestamp = Date.now()
  return {
    id: DEFAULT_PROVIDER_ID,
    type: 'browseros',
    name: getBuiltInProviderName(),
    baseUrl: 'https://api.thriveos.app/v1',
...
export function createDefaultProvidersConfig(): LlmProviderConfig[] {
  return [createDefaultThriveOSProvider()]
}
