...
Monorepo for the ThriveOS-agent -- contains 3 packages: agent-UI, server (which contains the agent loop) and controller-extension (which is used by the tools within the agent loop).

> **⚠️ NOTE:** This is only a submodule, the main project is at -- https://github.com/browseros-ai/ThriveOS
...
apps/
  ...
  controller-ext/  # ThriveOS Controller (Chrome extension for chrome.* APIs)
...
| `apps/controller-ext` | ThriveOS Controller - Chrome extension that bridges `chrome.*` APIs (tabs, bookmarks, history) to the server via WebSocket |
...
- `apps/controller-ext`: ThriveOS Controller - a Chrome extension that bridges `chrome.*` APIs to the server. Controller tools within the server communicate with this extension via WebSocket.
...
└──────────────────────────────────────────────────────────────────────────┘
                                    ...
                                    ▼
┌──────────────────────────────────────────────────────────────────────────┐
│                 ThriveOS Server (serverPort: 9100)                      │
│                                                                          │
...
└──────────────────────────────────────────────────────────────────────────┘
          ...
          ▼                                         ▼
┌─────────────────────┐              ┌─────────────────────────────────────┐
│   Chromium CDP      │              │   ThriveOS Controller Extension    │
...
| 9000 | `BROWSEROS_CDP_PORT` | Chromium CDP server (ThriveOS Server connects as client) |
...
| Variable | Default | Description |
|----------|---------|-------------|
| `BROWSEROS_SERVER_PORT` | 9100 | Passed to ThriveOS via CLI args |
| `BROWSEROS_CDP_PORT` | 9000 | Passed to ThriveOS via CLI args |
| `BROWSEROS_EXTENSION_PORT` | 9300 | Passed to ThriveOS via CLI args |
| `VITE_BROWSEROS_SERVER_PORT` | 9100 | Agent UI connects to server (must match `BROWSEROS_SERVER_PORT`) |
| `BROWSEROS_BINARY` | - | Path to ThriveOS binary |
| `USE_BROWSEROS_BINARY` | true | Use ThriveOS instead of default Chrome |
