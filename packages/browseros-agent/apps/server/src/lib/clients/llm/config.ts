...
import { fetchThriveOSConfig, getLLMConfigFromProvider } from '../gateway'
...
export async function resolveLLMConfig(
  ...
  // ThriveOS gateway: fetch config from remote service
  if (config.provider === LLM_PROVIDERS.BROWSEROS) {
    return resolveThriveOSConfig(config, browserosId)
  }
...
async function resolveThriveOSConfig(
  config: LLMConfig,
  browserosId?: string,
): Promise<ResolvedLLMConfig> {
  ...
  const browserosConfig = await fetchThriveOSConfig(configUrl, browserosId)
