...
**ThriveOS Server** - The automation engine inside ThriveOS. This MCP server powers the built-in AI agent and lets external tools like `claude-code` or `gemini-cli` control the browser. Starts automatically when ThriveOS launches.
...
# Testing
bun run test                     # Run tool tests (requires ThriveOS running)
...
**Entry point:** `src/background/index.ts` → `ThriveOSController`
...
- `tools/` - Tool tests (require ThriveOS running with CDP)
