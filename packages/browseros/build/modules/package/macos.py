#!/usr/bin/env python3
"""DMG creation and packaging module for ThriveOS"""
...
class MacOSPackageModule(CommandModule):
    ...
    def _create_dmg(self, app_path: Path, dmg_path: Path, pkg_dmg_path: Path) -> None:
        if not create_dmg(app_path, dmg_path, "ThriveOS", pkg_dmg_path):
    ...
    def _create_signed_notarized_dmg(
        ...
        if not create_signed_notarized_dmg(
            app_path, dmg_path, certificate_name, "ThriveOS", pkg_dmg_path, keychain_profile
        ):
            raise RuntimeError("Failed to create signed and notarized DMG")
def create_dmg(
    app_path: Path,
    dmg_path: Path,
    volume_name: str = "ThriveOS",
...
def package_universal(contexts: List[Context]) -> bool:
    ...
    if create_dmg(universal_app_path, dmg_path, "ThriveOS", pkg_dmg_path):
