...
export class Application {
  ...
  async start(): Promise<void> {
    logger.info(`Starting ThriveOS Server v${VERSION}`)
    ...
    const dailyRateLimit = await fetchDailyRateLimit(identity.getThriveOSId())
    ...
    try {
      await createHttpServer({
        ...
        controller,
        registry,
        browserosId: identity.getThriveOSId(),
  ...
  private async initCoreServices(): Promise<void> {
    ...
    const browserosId = identity.getThriveOSId()
    logger.info('ThriveOS ID initialized', {
