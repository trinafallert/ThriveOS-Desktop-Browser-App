#!/usr/bin/env python3
"""Application signing and notarization module for ThriveOS (macOS)"""
...
# Central list of ThriveOS Server binaries we need to sign explicitly.
...
def get_browseros_server_binary_info(component_path: Path) -> Optional[Dict[str, str]]:
    """Return metadata for known ThriveOS Server binaries, if applicable."""
...
class MacOSSignModule(CommandModule):
    ...
    def execute(self, ctx: Context) -> None:
        log_info("=" * 70)
        log_info("🚀 Starting signing process for ThriveOS...")
        log_info("=" * 70)
...
def find_components_to_sign(
    ...
    # Check both versioned and non-versioned paths for ThriveOS Framework
    # Handle both release and debug framework names
    framework_names = [
        "ThriveOS Framework.framework",
        "ThriveOS Dev Framework.framework",
    ]
    ...
    # Find all dylibs (check versioned path for ThriveOS Framework libraries)
    for nxtscape_fw_path in nxtscape_framework_paths:
    ...
    # Find ThriveOS Server binaries
    browseros_server_dir = join_paths(app_path, "Contents", "Resources", "ThriveOSServer")
...
def get_identifier_for_component(
    ...
    # ThriveOS Server binaries share the same entitlements/options but need unique identifiers.
    ...
    if component_path.suffix == ".framework":
        if name == "ThriveOS Framework" or name == "ThriveOS Dev Framework":
...
def get_signing_options(component_path: Path) -> str:
    ...
    # Known ThriveOS Server binaries share the same relaxed options.
    browseros_server_info = get_browseros_server_binary_info(component_path)
    if browseros_server_info:
...
def sign_all_components(
    ...
    # 6. Sign frameworks (except the main ThriveOS Framework)
    if components["frameworks"]:
        log_info("\n🔏 Signing frameworks...")
        # Sort to sign Sparkle.framework before ThriveOS Framework.framework
    ...
    main_exe_names = ["ThriveOS", "ThriveOS Dev"]
    main_exe = None
    ...
    if not sign_component(main_exe, certificate_name, "com.browseros.ThriveOS"):
        return False

    # 8. Finally sign the app bundle
    log_info("\n🔏 Signing application bundle...")
    requirements = (
        '=designated => identifier "com.browseros.ThriveOS" and '
