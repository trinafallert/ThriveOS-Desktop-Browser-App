...
+
+"""Validates that required ThriveOS resources exist.
+
...
+    print(f"ERROR: Required ThriveOS resource not found: {resource_path}")
+    all_valid = False
+    continue
+
...
+  sys.exit(1)
+
+print(f"✓ ThriveOS resources validated ({len(REQUIRED_RESOURCES)} resources)")
+sys.exit(0)
