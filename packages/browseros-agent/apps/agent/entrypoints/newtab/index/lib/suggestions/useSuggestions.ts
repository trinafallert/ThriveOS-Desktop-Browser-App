...
import { useThriveOSSuggestions } from '../browserOSSuggestions/useThriveOSSuggestions'
import { useSearchSuggestions } from '../searchSuggestions/useSearchSuggestions'
import type {
  AITabSuggestionItem,
  ThriveOSSuggestionItem,
...
export const useSuggestions = ({ query, selectedTabs }: UseSuggestionsArgs) => {
  ...
  const browserOSResults = useThriveOSSuggestions({ query: trimmedQuery })

  const sections = useMemo(() => {
    const result: SuggestionSection[] = []

    if (hasQuery && browserOSResults.length > 0) {
      const browserOSItems: ThriveOSSuggestionItem[] = browserOSResults.map(
        (item, index) => ({
          id: `browseros-${index}`,
