...
import { createThriveOSFetch } from '../../browseros-fetch'
...
function createThriveOSModel(config: ResolvedLLMConfig): LanguageModel {
  if (!config.baseUrl) throw new Error('ThriveOS provider requires baseUrl')
  const { baseUrl, apiKey, model, upstreamProvider, browserosId } = config
  const browserosFetch = browserosId
    ? createThriveOSFetch(browserosId)
  ...
  logger.debug('Creating OpenAI-compatible provider for ThriveOS')
  return createOpenAICompatible({
    name: 'browseros',
    baseURL: baseUrl,
    ...(apiKey && { apiKey }),
...
const PROVIDER_FACTORIES: Record<string, ProviderFactory> = {
  ...
  [LLM_PROVIDERS.BROWSEROS]: createThriveOSModel,
